...
def main():
    ...
    ap.add_argument("--task-files", default=None,
                    help="directory holding the released task_*.jsonl "
                         "(default: $SHIPBENCH_ROOT/task_files)")
    args = ap.parse_args()

    # Load predictions
    opus_preds = [json.loads(l) for l in open(args.opus, encoding="utf-8")]
    gpt_preds = [json.loads(l) for l in open(args.gpt5, encoding="utf-8")]

    opus_by_id = {p["qa_id"]: p for p in opus_preds}
    gpt_by_id = {p["qa_id"]: p for p in gpt_preds}

    # Ground truth. Resolved from --task-files, else $SHIPBENCH_ROOT/task_files.
    # Earlier revisions carried the literal string "<SHIPBENCH_ROOT>" here, left
    # over from path anonymisation, so no ground-truth file was ever found and
    # the per-task section came out empty.
    tf = args.task_files or os.path.join(os.environ.get("SHIPBENCH_ROOT", ""),
                                         "task_files")
    gt_files = sorted(glob.glob(os.path.join(tf, "task_*.jsonl")))
    if not gt_files:
        sys.exit("ERROR: no task_*.jsonl found under %r.\n"
                 "  Set SHIPBENCH_ROOT to the dataset root, or pass "
                 "--task-files <dir>." % tf)
    gt_by_id = {}
    for f in gt_files:
    ...
    print("Ground truth: %d items from %d files under %s"
          % (len(gt_by_id), len(gt_files), tf))
